...
import { aggregate, aggregateGroups, asOf, groupBy, groupSeries, unionGrid } from "./aggregate.ts";
...
test("groupSeries draws members, band and mean per (metric, group)", () => {
  ...
  // Everything in group A shares one colour; B and the ungrouped run get the next ones.
