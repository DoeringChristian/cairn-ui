 ...
 *  for the full pattern writeup (identical mechanics, pointcloud-specific
 *  diff math). `primaryHash`/`primaryMeta` are resolved by the orchestrator
 *  (`PointCloudComparePanel`, below) from data already fetched for every
 *  pane; only the reference series' own points are fetched here (metadata
 *  lookup for the hash `useMediaReference` already resolved). */
function PointCloudComparePane({
  runId,
  primaryHash,
  primaryMeta,
  referenceTag,
 ...
 *  full pattern writeup (identical mechanics, pointcloud-specific pane). */
  ...
  return (
    <MultiPaneGrid
      ...
      renderPane={(_key, i) => {
        ...
        const primaryMeta = safeJsonParse<PointCloudMeta>(primaryPoint?.artifact_metadata);
...
export default function PointCloudCard({
  ...
  // Topology-select proxy — cheap when a reference was DRAGGED IN
  // (assume OK; the render path is authoritative), full check otherwise.
  ...
  const compareTopologyOk =
    hasExternalRef ||
    (!!meta && !!referenceMetaForCompare && meta.n_points === referenceMetaForCompare.n_points);
