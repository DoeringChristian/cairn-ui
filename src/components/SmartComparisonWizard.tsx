...
export default function SmartComparisonWizard({
  ...
  return (
    <Dialog
      open={open}
      onClose={onClose}
      title={step === "preview" ? "Smart Comparison — Preview" : "Smart Comparison"}
      size="3xl"
      fill
    >
      {step === "filters" ? (
        <>
          {/* Strategy */}
          <div className="shrink-0 border-b border-border px-4 py-3 flex flex-wrap items-center gap-x-4 gap-y-1">
            <label className="text-xs text-fg-muted">Strategy:</label>
            <label className="inline-flex items-center gap-1 text-xs touch:min-h-10">
              <input
                type="radio"
                name="strategy"
                        ...
                        <div key={f.key} className="rounded border border-border p-3">
                          <div className="flex items-center justify-between gap-2 mb-2">
                            <div className="flex min-w-0 items-center gap-2">
                              <span className="mono min-w-0 truncate text-xs font-semibold">{f.key}</span>
                              <button
                                type="button"
                                onClick={() => toggleFilterMode(f.key)}
                                className="shrink-0 rounded border border-border-subtle px-1.5 py-0.5 text-[10px] text-fg-muted hover:border-accent hover:text-fg transition-colors touch:min-h-10 touch:px-3"
                            ...
                            <button
                              ...
                              className="shrink-0 text-xs text-fg-subtle hover:text-fg touch:min-h-10 touch:px-2"
                            >
                              {"\u00D7"} Remove
                            </button>
                          </div>
                                ...
                                {allValues.map((val) => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      className={`mono max-w-full break-all rounded border px-2 py-0.5 text-left text-xs transition-colors touch:min-h-10 ${
                                        selected
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {paramKeys.filter((k) => !selectedFilterKeys.has(k)).map((key) => (
                      <button
                        ...
                        className="mono max-w-full break-all rounded border border-border-subtle px-2 py-1 text-left text-xs text-fg-muted hover:border-accent hover:text-fg transition-colors touch:min-h-10"
                      >
                        + {key}
                      </button>
                    ))}
          ...
          <DialogFooter>
            ...
            <button
              ...
              className="btn text-xs px-4 touch:min-h-10"
            >
              Preview {"\u2192"}
            </button>
          </DialogFooter>
        ...
        <>
          ...
          <DialogFooter>
            ...
            <button
              ...
              className="btn text-xs px-4 bg-accent text-white hover:bg-accent/90 disabled:opacity-50 touch:min-h-10"
            >
