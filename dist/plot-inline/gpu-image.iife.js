...
`;function he(e){switch(e){case"rgba8unorm":return"rgba8unorm";case"rgba16float":return"rgba16float";case"rgba32float":return"rgba32float";case"r32float":return"r32float";default:{const t=e;throw new Error(`webgpu device: unknown TextureFormat ${String(t)}`)}}}function Pe(e){switch(e){case"rgba8unorm":return 4;case"rgba16float":return 8;case"rgba32float":return 16;case"r32float":return 4;default:{const t=e;throw new Error(`webgpu device: unknown TextureFormat ${String(t)}`)}}}function pt(e){const t=(e&32768)>>15,r=(e&31744)>>10,n=e&1023;let o;return r===0?o=n/1024*Math.pow(2,-14):r===31?o=n?NaN:1/0:o=(1+n/1024)*Math.pow(2,r-15),t?-o:o}const gt={texture:0,sampler:1,uniform:2};function be(e,t){return e*3+gt[t]}const ht={f32:4,i32:4,u32:4,"vec2<f32>":8,vec2f:8,"vec3<f32>":12,vec3f:12,"vec4<f32>":16,vec4f:16,"mat4x4<f32>":64,mat4x4f:64};function bt(e){const t=new Map,r=/@group\(0\)\s*@binding\((\d+)\)\s*var(<uniform>)?\s+\w+\s*:\s*([^;]+);/g;let n;for(;(n=r.exec(e))!==null;){const o=Number(n[1]),a=n[2]!==void 0,v=n[3].trim();if(a){const h=ht[v];if(h===void 0)throw new Error(`webgpu device: parseWGSLBindings doesn't know the size of uniform type "${v}" (binding ${o}). Add it to WGSL_UNIFORM_TYPE_SIZE.`);t.set(o,{kind:"uniform",sizeBytes:h})}else v==="sampler"||v==="sampler_comparison"?t.set(o,{kind:"sampler"}):t.set(o,{kind:"texture"})}return t}class Ae{constructor(t,r,n,o){M(this,"width");M(this,"height");M(this,"format");M(this,"gpuTexture");M(this,"device");M(this,"destroyed",!1);this.device=t,this.width=r,this.height=n,this.format=o,this.gpuTexture=t.createTexture({size:{width:r,height:n,depthOrArrayLayers:1},format:he(o),usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_DST|GPUTextureUsage.COPY_SRC|GPUTextureUsage.RENDER_ATTACHMENT})}write(t){if(this.destroyed)throw new Error("webgpu device: write() on a destroyed texture");const r=this.width*Pe(this.format);this.device.queue.writeTexture({texture:this.gpuTexture},t,{bytesPerRow:r,rowsPerImage:this.height},{width:this.width,height:this.height,depthOrArrayLayers:1})}destroy(){this.destroyed||(this.gpuTexture.destroy(),this.destroyed=!0)}}class Me{constructor(t){M(this,"_s");M(this,"gpuSampler");this.gpuSampler=t,this._s=t}}class vt{constructor(t,r,n,o,a){M(this,"_p");M(this,"gpuPipeline");M(this,"bindings");M(this,"bindGroupLayout");M(this,"variants");M(this,"buildVariant");this.gpuPipeline=t,this.bindings=r,this.bindGroupLayout=n,this.buildVariant=a,this.variants=new Map([[o,t]]),this._p=t}pipelineFor(t){let r=this.variants.get(t);return r||(r=this.buildVariant(t),this.variants.set(t,r)),r}}function xt(e,t){const r=[];for(const[n,o]of t)o.kind==="uniform"?r.push({binding:n,visibility:GPUShaderStage.VERTEX|GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}}):o.kind==="sampler"?r.push({binding:n,visibility:GPUShaderStage.FRAGMENT,sampler:{type:"filtering"}}):r.push({binding:n,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"unfilterable-float"}});return e.createBindGroupLayout({entries:r})}class wt{constructor(t){M(this,"_c");M(this,"gpuPipeline");this.gpuPipeline=t,this._c=t}}class Et{constructor(t,r){M(this,"_b");M(this,"gpuBindGroup");M(this,"ownedBuffers");M(this,"destroyed",!1);this.gpuBindGroup=t,this.ownedBuffers=r,this._b=t}destroy(){if(!this.destroyed){for(const t of this.ownedBuffers)t.destroy();this.destroyed=!0}}}class _t{constructor(t,r,n,o){M(this,"canvas");M(this,"hdr");M(this,"format");M(this,"context");M(this,"reconfigure");this.canvas=t,this.context=r,this.hdr=n.hdr,this.format=n.format,this.reconfigure=o}configure(t,r){this.canvas.width=t,this.canvas.height=r;const n=this.reconfigure();this.hdr=n.hdr,this.format=n.format}getCurrentTextureView(){return this.context.getCurrentTexture().createView()}getCurrentGPUTexture(){return this.context.getCurrentTexture()}}function de(e){return"canvas"in e}async function yt(){if(!("gpu"in navigator)||!navigator.gpu)throw new Error("webgpu device: navigator.gpu is not available in this browser");const e=await navigator.gpu.requestAdapter();if(!e)throw new Error("webgpu device: requestAdapter() returned null");const t=await e.requestDevice(),r={hdr:!0,compute:!0,float16:!0};let n=null;function o(){return n||(n=t.createSampler({magFilter:"nearest",minFilter:"nearest",addressModeU:"clamp-to-edge",addressModeV:"clamp-to-edge"})),n}function a(i){return de(i)?i.getCurrentTextureView():i.gpuTexture.createView()}function v(i){if(de(i))return{width:i.canvas.width,height:i.canvas.height};const p=i;return{width:p.width,height:p.height}}let h=!1;const f=256;let w=null,c=null;function m(){if(!w||!c){const i=t.createShaderModule({code:mt});c=t.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,texture:{sampleType:"unfilterable-float"}},{binding:1,visibility:GPUShaderStage.COMPUTE,texture:{sampleType:"unfilterable-float"}},{binding:2,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:3,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}}]});const p=t.createPipelineLayout({bindGroupLayouts:[c]});w=t.createComputePipeline({layout:p,compute:{module:i,entryPoint:"cs_main"}})}return{pipeline:w,layout:c}}return{backend:"webgpu",capabilities:r,createTexture(i,p,s){return new Ae(t,i,p,s)},createSampler(i){const p=(i==null?void 0:i.filter)==="linear"?"linear":"nearest",s=t.createSampler({magFilter:p,minFilter:p,addressModeU:"clamp-to-edge",addressModeV:"clamp-to-edge"});return new Me(s)},createRenderPipeline(i){const p=t.createShaderModule({code:i.shaderWGSL}),s=bt(i.shaderWGSL),u=he(i.targetFormat),g=xt(t,s),b=t.createPipelineLayout({bindGroupLayouts:[g]}),d=T=>t.createRenderPipeline({layout:b,vertex:{module:p,entryPoint:"vs_main"},fragment:{module:p,entryPoint:"fs_main",targets:[{format:T}]},primitive:{topology:"triangle-list"}}),y=d(u);return new vt(y,s,g,u,d)},createComputePipeline(i){const p=t.createShaderModule({code:i.shaderWGSL}),s=t.createComputePipeline({layout:"auto",compute:{module:p,entryPoint:"cs_main"}});return new wt(s)},createBindGroup(i,p){const s=i,u=new Map,g=[];for(const[d,y]of s.bindings)if(y.kind==="uniform"){const T=t.createBuffer({size:y.sizeBytes,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});g.push(T),u.set(d,{binding:d,resource:{buffer:T}})}else y.kind==="sampler"&&u.set(d,{binding:d,resource:o()});for(const d of p){const y=d.resource;if(y instanceof Ae){const T=be(d.binding,"texture");s.bindings.has(T)&&u.set(T,{binding:T,resource:y.gpuTexture.createView()})}else if(y instanceof Me){const T=be(d.binding,"sampler");s.bindings.has(T)&&u.set(T,{binding:T,resource:y.gpuSampler})}else{const T=be(d.binding,"uniform"),A=s.bindings.get(T);if(A&&A.kind==="uniform"){const I=y.uniform,G=t.createBuffer({size:Math.max(A.sizeBytes,I.byteLength),usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});t.queue.writeBuffer(G,0,I.buffer,I.byteOffset,I.byteLength),g.push(G),u.set(T,{binding:T,resource:{buffer:G}})}}}const b=t.createBindGroup({layout:s.bindGroupLayout,entries:Array.from(u.values())});return new Et(b,g)},createSurface(i,p){const s=i.getContext("webgpu");if(!s)throw new Error("webgpu device: canvas.getContext('webgpu') returned null");const u=p.hdr&&r.hdr,g=()=>u?ft(s,t):Ce(s,t),b=g();return new _t(i,s,b,g)},renderFullscreen(i,p,s){const u=p,g=s,b=a(i),{width:d,height:y}=v(i),T=de(i)?i.format:he(i.format),A=u.pipelineFor(T),I=t.createCommandEncoder(),G=I.beginRenderPass({colorAttachments:[{view:b,loadOp:"clear",clearValue:{r:0,g:0,b:0,a:0},storeOp:"store"}]});G.setPipeline(A),G.setBindGroup(0,g.gpuBindGroup),G.setViewport(0,0,d,y,0,1),G.draw(3),G.end(),t.queue.submit([I.finish()])},async readback(i){const p=de(i),{width:s,height:u}=v(i),g=p?i.hdr?"rgba16float":"rgba8unorm":i.format,b=p&&i.format==="bgra8unorm",d=p?i.getCurrentGPUTexture():i.gpuTexture,y=Pe(g),T=s*y,A=256,I=Math.ceil(T/A)*A,G=I*u,O=t.createBuffer({size:G,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ}),B=t.createCommandEncoder();B.copyTextureToBuffer({texture:d},{buffer:O,bytesPerRow:I,rowsPerImage:u},{width:s,height:u,depthOrArrayLayers:1}),t.queue.submit([B.finish()]),await O.mapAsync(GPUMapMode.READ);const V=new Uint8Array(O.getMappedRange()),L=new Uint8Array(T*u);for(let U=0;U<u;U++){const z=U*I,N=U*T;L.set(V.subarray(z,z+T),N)}if(O.unmap(),O.destroy(),g==="rgba8unorm"){if(b)for(let U=0;U<L.length;U+=4){const z=L[U],N=L[U+2];L[U]=N,L[U+2]=z}return L}if(g==="rgba16float"){const U=new Uint16Array(L.buffer,L.byteOffset,L.byteLength/2),z=new Float32Array(U.length);for(let N=0;N<U.length;N++)z[N]=pt(U[N]);return z}return new Float32Array(L.buffer,L.byteOffset,L.byteLength/4)},async reduceDiffSumSquaredAbs(i,p,s,u){const g=i,b=p,d=Math.max(0,s*u),y=Math.max(1,Math.ceil(d/f)),{pipeline:T,layout:A}=m(),I=y*2*4,G=t.createBuffer({size:I,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC}),O=t.createBuffer({size:16,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});t.queue.writeBuffer(O,0,new Uint32Array([Math.max(1,s),Math.max(1,u),d,0]));const B=t.createBindGroup({layout:A,entries:[{binding:0,resource:g.gpuTexture.createView()},{binding:1,resource:b.gpuTexture.createView()},{binding:2,resource:{buffer:G}},{binding:3,resource:{buffer:O}}]}),V=t.createBuffer({size:I,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ}),L=t.createCommandEncoder(),U=L.beginComputePass();U.setPipeline(T),U.setBindGroup(0,B),U.dispatchWorkgroups(y),U.end(),L.copyBufferToBuffer(G,0,V,0,I),t.queue.submit([L.finish()]),await V.mapAsync(GPUMapMode.READ);const N=new Float32Array(V.getMappedRange()).slice();V.unmap(),V.destroy(),G.destroy(),O.destroy();let Y=0,j=0;for(let $=0;$<y;$++)Y+=N[$*2],j+=N[$*2+1];return{sumSq:Y,sumAbs:j}},destroy(){h||(t.destroy(),h=!0)},isContextLost(){return!1}}}let ve=null;function St(){if(typeof location>"u")return!1;try{return new URLSearchParams(location.search).has("forceWebGL2")}catch{return!1}}async function Rt(e){if(!e&&typeof navigator<"u"&&"gpu"in navigator&&!!navigator.gpu)try{return await yt()}catch{}return pe()}function xe(e){if(!ve){const t=St();ve=Rt(t)}return ve}function Tt(e,t,r){return[e[0]+(t[0]-e[0])*r,e[1]+(t[1]-e[1])*r,e[2]+(t[2]-e[2])*r]}function Ct(e){const t=new Uint8Array(768);for(let r=0;r<256;r++){const o=r/255*(e.length-1),a=Math.floor(o),v=Math.min(a+1,e.length-1),h=o-a,[f,w,c]=Tt(e[a],e[v],h);t[r*3]=Math.round(f),t[r*3+1]=Math.round(w),t[r*3+2]=Math.round(c)}return t}const Ge={viridis:[[68,1,84],[59,82,139],[33,145,140],[94,201,98],[253,231,37]],"red-green":[[215,25,28],[255,255,255],[26,150,65]],"red-blue":[[215,25,28],[255,255,255],[44,123,182]]},Pt=new Set(["red-green","red-blue"]),Le=new Map;function Fe(e){let t=Le.get(e);if(!t){const r=Ge[e]??Ge.viridis;t=Ct(r),Le.set(e,t)}return t}function At(e,t,r="linear"){const n=Fe(t),o=new ImageData(e.width,e.height),a=e.data,v=o.data;for(let h=0;h<a.length;h+=4){const f=(a[h]+a[h+1]+a[h+2])/3;let w;r==="positive"?w=Math.round(128+f/255*127):w=Math.round(f),w=Math.max(0,Math.min(255,w)),v[h]=n[w*3],v[h+1]=n[w*3+1],v[h+2]=n[w*3+2],v[h+3]=a[h+3]}return o}function Ie(e){const t=new Map;return{get(r){return t.get(r)},set(r,n){if(t.size>=e){const o=t.keys().next().value;o!==void 0&&t.delete(o)}t.set(r,n)}}}const Ue=Ie(50);function Mt(e){return Ue.get(e)}function Gt(e,t){Ue.set(e,t)}const De=Ie(100);function Lt(e){return De.get(e)}function Ft(e,t){De.set(e,t)}async function Oe(e){const t=Lt(e);return t||new Promise(r=>{const n=new Image;n.onload=()=>{try{const o=document.createElement("canvas");o.width=n.naturalWidth,o.height=n.naturalHeight;const a=o.getContext("2d");if(!a){r(null);return}a.drawImage(n,0,0);const v=a.getImageData(0,0,o.width,o.height);Ft(e,v),r(v)}catch(o){console.warn("[cairn] loadImageData failed:",o),r(null)}},n.onerror=o=>{console.warn("[cairn] loadImageData: image failed to load:",e,o),r(null)},n.src=e})}function Be(e){return e<=32?4:e<=128?16:e<=512?64:e<=2048?256:512}function It({naturalWidth:e,naturalHeight:t,zoom:r=1,containerRef:n}){const o=Be(e),a=Be(t),v=[];for(let b=0;b<=e;b+=o)v.push(b);const h=[];for(let b=0;b<=t;b+=a)h.push(b);const f=1/r,w=8*f,c=-12*f,m=-2*f,x=n==null?void 0:n.current;let i=0,p=0,s=0,u=0;if(x){const b=x.clientWidth,d=x.clientHeight,y=b/e,T=d/t,A=Math.min(y,T);s=e*A,u=t*A,i=(b-s)/2,p=(d-u)/2}const g=x&&s>0;return S.jsxs(S.Fragment,{children:[S.jsx("div",{className:"absolute left-0 right-0 text-fg-muted leading-none pointer-events-none select-none",style:{top:g?p:0,transform:`translateY(${c}px)`,fontSize:w},children:v.map(b=>S.jsx("span",{className:"mono",style:{position:"absolute",left:g?i+b/e*s:`${b/e*100}%`,transform:"translateX(-50%)"},children:b},b))}),S.jsx("div",{className:"absolute top-0 bottom-0 text-fg-muted leading-none pointer-events-none select-none",style:{left:g?i:0,transform:`translateX(${m}px)`,fontSize:w},children:h.map(b=>S.jsx("span",{className:"mono",style:{position:"absolute",top:g?p+b/t*u:`${b/t*100}%`,transform:"translate(-100%, -50%)",paddingRight:`${3*f}px`},children:b},b))})]})}function Ut({label:e,isDraggable:t,onDragStart:r}){return S.jsxs("span",{className:`absolute bottom-1 left-1 z-10 rounded bg-bg/80 px-1 py-0.5 text-[10px] text-fg-muted backdrop-blur-sm flex items-center gap-1${t?" cairn-drag-grip":""}`,draggable:t,onDragStart:r,style:{cursor:t?"grab":void 0},children:[t&&S.jsx("i",{className:"fa-solid fa-grip-vertical text-[8px] opacity-50","aria-hidden":"true"}),e]})}const ke=["#0969da","#d29922","#3fb950","#f85149","#c678dd","#56d4dd"];function we(e){const t=ke.length;return ke[(e%t+t)%t]}function Dt(e){const r=l.useRef(null),[n,o]=l.useState({w:0,h:0}),a=l.useRef(null),v=l.useRef(null);return l.useEffect(()=>{var w;const h=r.current;if(h===v.current||((w=a.current)==null||w.disconnect(),a.current=null,v.current=h,!h))return;const f=new ResizeObserver(c=>{for(const m of c)o({w:m.contentRect.width,h:m.contentRect.height})});a.current=f,f.observe(h)}),l.useEffect(()=>()=>{var h;return(h=a.current)==null?void 0:h.disconnect()},[]),{ref:r,size:n}}function Ot(){const[e,t]=l.useState(!1);return l.useEffect(()=>{const r=a=>{(a.key==="Alt"||a.key==="Control"||a.key==="Meta")&&t(!0)},n=a=>{(a.key==="Alt"||a.key==="Control"||a.key==="Meta")&&t(!1)},o=()=>t(!1);return window.addEventListener("keydown",r),window.addEventListener("keyup",n),window.addEventListener("blur",o),()=>{window.removeEventListener("keydown",r),window.removeEventListener("keyup",n),window.removeEventListener("blur",o)}},[]),e}const Bt=.25,kt=16;function Ve(e){const{containerRef:t,zoom:r,pan:n,onViewportChange:o,minZoom:a=Bt,maxZoom:v=kt}=e,h=Ot(),f=l.useRef(h);f.current=h;const w=l.useRef({zoom:r,pan:n});w.current={zoom:r,pan:n};const c=l.useRef(o);c.current=o,l.useEffect(()=>{const u=t.current;if(!u||!o)return;const g=b=>{var V;if(!f.current)return;b.preventDefault(),b.stopPropagation();const d=b.deltaY<0?1.1:1/1.1,y=w.current,T=Math.max(a,Math.min(v,y.zoom*d));if(y.zoom===T)return;const A=u.getBoundingClientRect(),I=b.clientX-A.left,G=b.clientY-A.top,O=I-(I-y.pan.x)/y.zoom*T,B=G-(G-y.pan.y)/y.zoom*T;(V=c.current)==null||V.call(c,{zoom:T,pan:{x:O,y:B}})};return u.addEventListener("wheel",g,{passive:!1}),()=>u.removeEventListener("wheel",g)},[t,!!o,a,v]);const m=l.useRef(null),x=l.useCallback(u=>{!f.current||!c.current||(u.currentTarget.setPointerCapture(u.pointerId),m.current={pointerId:u.pointerId,startX:u.clientX,startY:u.clientY,panX:w.current.pan.x,panY:w.current.pan.y})},[]),i=l.useCallback(u=>{var y;const g=m.current;if(!g||g.pointerId!==u.pointerId)return;const b=u.clientX-g.startX,d=u.clientY-g.startY;(y=c.current)==null||y.call(c,{zoom:w.current.zoom,pan:{x:g.panX+b,y:g.panY+d}})},[]),p=l.useCallback(u=>{const g=m.current;if(!(!g||g.pointerId!==u.pointerId)){try{u.currentTarget.releasePointerCapture(u.pointerId)}catch{}m.current=null}},[]),s=h&&!!o;return{containerProps:{onPointerDown:x,onPointerMove:i,onPointerUp:p,onPointerCancel:p,style:{cursor:s?"move":void 0,touchAction:s?"none":void 0}},modifierActive:h}}function Vt(e){const t=e.replace("#","");return[parseInt(t.slice(0,2),16),parseInt(t.slice(2,4),16),parseInt(t.slice(4,6),16)]}function Ne(e,t,r){return!(r.has(e.class_id)||e.score!=null&&e.score<t.scoreThreshold)}function Nt({data:e,settings:t,naturalWidth:r,naturalHeight:n}){const{ref:o,size:a}=Dt(),v=l.useRef(null),h=l.useMemo(()=>new Set(t.hiddenClasses),[t.hiddenClasses]),f=l.useMemo(()=>{const s=a.w,u=a.h;if(s<=0||u<=0||r<=0||n<=0)return null;const g=Math.min(s/r,u/n),b=r*g,d=n*g;return{left:(s-b)/2,top:(u-d)/2,width:b,height:d}},[a.w,a.h,r,n]),w=e.masks,c=t.showMasks&&!!w&&w.length>0,m=l.useMemo(()=>t.hiddenClasses.join(","),[t.hiddenClasses]);if(l.useEffect(()=>{if(!c||!w)return;const s=v.current;if(!s)return;(s.width!==r||s.height!==n)&&(s.width=r,s.height=n);const u=s.getContext("2d");if(!u)return;u.clearRect(0,0,s.width,s.height);let g=!1;const b=u.createImageData(r,n),d=b.data;let y=w.length,T=!1;const A=()=>{g||T&&u.putImageData(b,0,0)},I=document.createElement("canvas");I.width=r,I.height=n;const G=I.getContext("2d",{willReadFrequently:!0});for(const O of w){const B=new Image;B.onload=()=>{if(!g){if(G){G.clearRect(0,0,r,n),G.drawImage(B,0,0,r,n);const V=G.getImageData(0,0,r,n).data;for(let L=0;L<r*n;L++){const U=V[L*4];if(U===0||h.has(U))continue;const[z,N,Y]=Vt(we(U));d[L*4]=z,d[L*4+1]=N,d[L*4+2]=Y,d[L*4+3]=255,T=!0}}y-=1,y===0&&A()}},B.onerror=()=>{y-=1,y===0&&A()},B.src=`data:image/png;base64,${O.png_b64}`}return()=>{g=!0}},[c,w,r,n,m]),!f)return S.jsx("div",{ref:o,className:"absolute inset-0 pointer-events-none"});const x=e.boxes??[],i=t.showBoxes&&x.length>0,p=e.class_labels??{};return S.jsxs("div",{ref:o,className:"absolute inset-0 pointer-events-none overflow-hidden",children:[c&&S.jsx("canvas",{ref:v,className:"absolute",style:{left:f.left,top:f.top,width:f.width,height:f.height,opacity:t.maskOpacity,imageRendering:"pixelated"}}),i&&S.jsx("svg",{className:"absolute",style:{left:f.left,top:f.top,width:f.width,height:f.height,overflow:"visible"},viewBox:`0 0 ${r} ${n}`,preserveAspectRatio:"none",children:x.map((s,u)=>{if(!Ne(s,t,h))return null;const g=s.domain==="pixel"?1:r,b=s.domain==="pixel"?1:n,d=s.position.minX*g,y=s.position.minY*b,T=(s.position.maxX-s.position.minX)*g,A=(s.position.maxY-s.position.minY)*b;return S.jsx("rect",{x:d,y,width:T,height:A,fill:"none",stroke:we(s.class_id),strokeWidth:2,vectorEffect:"non-scaling-stroke"},u)})}),i&&S.jsx("div",{className:"absolute",style:{left:f.left,top:f.top,width:f.width,height:f.height},children:x.map((s,u)=>{if(!Ne(s,t,h))return null;const g=s.domain==="pixel"?1/r:1,b=s.domain==="pixel"?1/n:1,d=s.position.minX*g*100,y=s.position.minY*b*100,T=s.label??p[String(s.class_id)]??`#${s.class_id}`,A=s.score!=null?` ${(s.score*100).toFixed(0)}%`:"";return!T&&!A?null:S.jsx("span",{className:"absolute whitespace-nowrap rounded px-1 text-[10px] leading-tight text-white",style:{left:`${d}%`,top:`${y}%`,transform:"translateY(-100%)",backgroundColor:we(s.class_id)},children:S.jsxs("span",{className:"mono",children:[T,A]})},u)})})]})}const zt=30,ee=["#ff5a5a","#39d353","#5b9bff"];function Ee(e){if(!Number.isFinite(e))return"0";const t=Math.abs(e);return t!==0&&(t<.001||t>=1e4)?e.toExponential(1):String(Number(e.toPrecision(3)))}function Z(e,t,r){return t==="uint8"?r==="int"?String(Math.round(e)):Ee(e/255):Ee(r==="int"?e*255:e)}function fe({imageElRef:e,naturalWidth:t,naturalHeight:r,zoom:n,pan:o,sample:a,notation:v="decimal",version:h=0,onActiveChange:f}){const w=l.useRef(null),c=l.useRef(!1),m=l.useRef(f);m.current=f;const x=l.useCallback(p=>{var s;p!==c.current&&(c.current=p,(s=m.current)==null||s.call(m,p))},[]),i=l.useCallback(()=>{var j;const p=w.current,s=e.current;if(!p)return;const u=window.devicePixelRatio||1,g=p.clientWidth,b=p.clientHeight;if(g===0||b===0)return;p.width!==Math.round(g*u)&&(p.width=Math.round(g*u)),p.height!==Math.round(b*u)&&(p.height=Math.round(b*u));const d=p.getContext("2d");if(!d)return;if(d.setTransform(u,0,0,u,0,0),d.clearRect(0,0,g,b),!s||t<=0||r<=0){x(!1);return}const y=s.getBoundingClientRect(),T=p.getBoundingClientRect();if(y.width===0||y.height===0){x(!1);return}const A=Math.min(y.width/t,y.height/r);if(A<zt){x(!1);return}const I=t*A,G=r*A,O=y.left+(y.width-I)/2-T.left,B=y.top+(y.height-G)/2-T.top,V=Math.max(0,Math.floor((0-O)/A)),L=Math.min(t,Math.ceil((g-O)/A)),U=Math.max(0,Math.floor((0-B)/A)),z=Math.min(r,Math.ceil((b-B)/A));if(L<=V||z<=U){x(!1);return}x(!0),d.textAlign="center",d.textBaseline="middle",d.lineJoin="round";const N=A*.14,Y=A-N*2;for(let $=U;$<z;$++)for(let te=V;te<L;te++){const W=a(te,$,v);if(!W||W.lines.length===0)continue;const J=W.lines.length;let ae=1;for(const D of W.lines)D.length>ae&&(ae=D.length);const ue=Y/(J*1.15),Q=Y/(ae*.62)||ue,re=Math.min(ue,Q,24);if(re<6)continue;const F=O+(te+.5)*A,E=B+($+.5)*A,C=re*1.15,_=W.luminance<=.55,R=_?"#ffffff":"#000000";d.font=`${re}px ui-monospace, SFMono-Regular, Menlo, monospace`,d.lineWidth=Math.max(1.4,re*.16),d.strokeStyle=_?"rgba(0,0,0,0.85)":"rgba(255,255,255,0.9)";let P=E-J*C/2+C/2;for(let D=0;D<W.lines.length;D++){const k=W.lines[D];d.strokeText(k,F,P),d.fillStyle=((j=W.colors)==null?void 0:j[D])??R,d.fillText(k,F,P),P+=C}}},[e,t,r,a,v,x]);return l.useEffect(()=>{i()},[i,n,o.x,o.y,h,v]),l.useEffect(()=>{const p=w.current;if(!p)return;const s=new ResizeObserver(()=>i());return s.observe(p),()=>s.disconnect()},[i]),S.jsx("canvas",{ref:w,className:"absolute inset-0 w-full h-full pointer-events-none z-10","aria-hidden":!0})}function ze({notation:e,onChange:t,className:r=""}){return S.jsx("button",{type:"button",onClick:n=>{n.stopPropagation(),t(e==="int"?"decimal":"int")},onPointerDown:n=>n.stopPropagation(),className:`absolute top-1 right-1 z-20 rounded bg-bg/80 px-1.5 py-0.5 text-[10px] font-mono text-fg-muted backdrop-blur-sm hover:text-fg ${r}`,title:"Pixel-value notation: 0–255 integer (255 = white) vs 0–1 float (1.0 = white)",children:e==="int"?"0–255":"0–1"})}const Xt=`
struct VSOut {
...
`,_e={linear:0,srgb:1,reinhard:2,aces:3},Xe=new WeakMap;function Yt(e,t){let r=Xe.get(e);r||(r=new Map,Xe.set(e,r));let n=r.get(t);return n||(n=e.createRenderPipeline({shaderWGSL:Xt,shaderGLSL:Wt,targetFormat:t}),r.set(t,n)),n}function We(e){return"canvas"in e?e.hdr?"rgba16float":"rgba8unorm":e.format}function $e(e,t){if(t){if(t.length!==256*4)throw new Error(`renderImage: params.colormap must have exactly 256*4=1024 floats (256x4 RGBA LUT), got ${t.length}`);const n=e.createTexture(256,1,"rgba32float");return n.write(t),n}const r=e.createTexture(1,1,"rgba32float");return r.write(new Float32Array([0,0,0,1])),r}function Ht(e,t,r,n){var i;const o=We(t),a=Yt(e,o),v=$e(e,n.isScalar?n.colormap:void 0),h=typeof n.gamma=="number"&&n.gamma>0?n.gamma:0,f=_e[n.operator]??_e.srgb,w=new Float32Array([n.exposureEV,f,h,n.isScalar?1:0]),c=new Float32Array([n.uv.x,n.uv.y,n.uv.w,n.uv.h]),m=new Float32Array([n.hdrOut?1:0]);let x;try{x=e.createBindGroup(a,[{binding:0,resource:r},{binding:1,resource:v},{binding:2,resource:{uniform:w}},{binding:3,resource:{uniform:c}},{binding:4,resource:{uniform:m}}]),e.renderFullscreen(t,a,x)}finally{(i=x==null?void 0:x.destroy)==null||i.call(x),v.destroy()}}const Kt={signed:0,absolute:1,squared:2,relative_signed:3,relative_absolute:4,relative_squared:5},Zt={linear:0,signed:1,positive:2},jt={split:0,blend:1,diff:2},qe=new WeakMap;function Jt(e,t){let r=qe.get(e);r||(r=new Map,qe.set(e,r));let n=r.get(t);return n||(n=e.createRenderPipeline({shaderWGSL:$t,shaderGLSL:qt,targetFormat:t}),r.set(t,n)),n}function Qt(e,t,r,n,o){var g;const a=We(t),v=Jt(e,a),h=o.mode==="diff"&&!!o.diffColormap,f=h?o.diffColormap:void 0,w=$e(e,f),c=o.gamma,m=_e[o.operator],x=new Float32Array([o.exposureEV,m,c,0]),i=new Float32Array([o.uv.x,o.uv.y,o.uv.w,o.uv.h]),p=new Float32Array([jt[o.mode],o.split,o.alpha,Kt[o.diffSubmode]??0]),s=new Float32Array([Zt[o.diffCmapMode??"linear"]??0,0,h?1:0,0]);let u;try{u=e.createBindGroup(v,[{binding:0,resource:r},{binding:1,resource:n},{binding:2,resource:w},{binding:3,resource:{uniform:x}},{binding:4,resource:{uniform:i}},{binding:5,resource:{uniform:p}},{binding:6,resource:{uniform:s}}]),e.renderFullscreen(t,v,u)}finally{(g=u==null?void 0:u.destroy)==null||g.call(u),w.destroy()}}function Ye(e,t,r){if(r<=0)return{mse:0,psnr:1/0,mae:0};const n=e/r,o=t/r,a=n<=0?1/0:10*Math.log10(1/n);return{mse:n,psnr:a,mae:o}}async function er(e,t,r){const n=Math.min(t.width,r.width),o=Math.min(t.height,r.height),a=n*o*3;if(a<=0)return{mse:0,psnr:1/0,mae:0};if(e.reduceDiffSumSquaredAbs){const{sumSq:x,sumAbs:i}=await e.reduceDiffSumSquaredAbs(t,r,n,o);return Ye(x,i,a)}const v=await e.readback(t),h=await e.readback(r),f=v instanceof Uint8Array,w=h instanceof Uint8Array;let c=0,m=0;for(let x=0;x<o;x++)for(let i=0;i<n;i++){const p=(x*t.width+i)*4,s=(x*r.width+i)*4;for(let u=0;u<3;u++){const g=(v[p+u]??0)/(f?255:1),b=(h[s+u]??0)/(w?255:1),d=g-b;c+=d*d,m+=Math.abs(d)}}return Ye(c,m,a)}const tr=12,oe=[];function He(e){const t=oe.indexOf(e);t!==-1&&oe.splice(t,1),oe.push(e)}function rr(e){const t=oe.indexOf(e);t!==-1&&oe.splice(t,1)}function me(e){e.parked||(rr(e),e.srcTexture&&(e.srcTexture.destroy(),e.srcTexture=null),e.surface=null,e.device&&e.device!==e.sharedDevice&&e.device.destroy(),e.device=null,e.parked=!0)}function Ke(e){for(;oe.length>tr;){const t=oe.find(r=>r!==e&&!r.visible)??oe.find(r=>r!==e);if(!t)break;me(t)}}function Ze(e){if(e.disposed)return;if(!e.parked&&e.surface){He(e),Ke(e);return}const t=e.sharedDevice.backend==="webgl2"?pe():e.sharedDevice;if(e.device=t,e.surface=t.createSurface(e.canvas,{hdr:e.hdr}),e.source){e.canvas.width=e.source.width,e.canvas.height=e.source.height,e.surface.configure(e.source.width,e.source.height);const r=t.createTexture(e.source.width,e.source.height,e.source.format);r.write(e.source.data),e.srcTexture=r}e.parked=!1,He(e),Ke(e)}const nr=30;function je(e,t){if(!(e.disposed||!e.source)&&(Ze(e),!(!e.device||!e.surface||!e.srcTexture))){if(e.device.isContextLost()){Je(e,t);return}try{Ht(e.device,e.surface,e.srcTexture,t),e.restoreRetries=0}catch(r){if(e.device.isContextLost()){Je(e,t);return}throw r}}}function Je(e,t){if(!e.disposed){if(e.restoreRetries>=nr){e.restoreRetries=0;return}e.restoreRetries++,me(e),requestAnimationFrame(()=>je(e,t))}}function or(e){return{canvas:e.canvas,backend:e.sharedDevice.backend,get isParked(){return e.parked},setSource(t){if(!e.disposed&&(e.source=t,!e.parked&&e.device&&e.surface)){e.canvas.width=t.width,e.canvas.height=t.height,e.surface.configure(t.width,t.height),e.srcTexture&&e.srcTexture.destroy();const r=e.device.createTexture(t.width,t.height,t.format);r.write(t.data),e.srcTexture=r}},render(t){je(e,t)},park(){e.disposed||me(e)},restore(){e.disposed||!e.source||Ze(e)},setVisible(t){e.disposed||(e.visible=t)},dispose(){e.disposed||(me(e),e.source=null,e.disposed=!0)}}}async function ir(e,t){const r=await xe();return or({canvas:e,sharedDevice:r,device:null,hdr:!1,surface:null,srcTexture:null,source:null,parked:!0,disposed:!1,visible:!0,restoreRetries:0})}function Qe(e){e.dispose()}function ar(e){return"hdr"in e&&e.hdr!=null}const sr=["linear","srgb","reinhard","aces"];function cr(e){return e&&sr.includes(e)?e:"srgb"}const ie=e=>Number.isFinite(e)?e:0;function ur(e){if(e.length===2)return{h:e[0],w:e[1],c:1};if(e.length===3)return{h:e[0],w:e[1],c:e[2]};throw new Error(`GpuImagePane: unsupported HDR shape [${e.join(",")}] (want [H,W] or [H,W,C]).`)}function lr(e){const{h:t,w:r,c:n}=ur(e.shape),o=e.data,a=new Float32Array(r*t*4);for(let v=0;v<r*t;v++){const h=v*n;let f,w,c,m=1;n===1?f=w=c=ie(o[h]):n===3?(f=ie(o[h]),w=ie(o[h+1]),c=ie(o[h+2])):(f=ie(o[h]),w=ie(o[h+1]),c=ie(o[h+2]),m=ie(o[h+3]));const x=v*4;a[x]=f,a[x+1]=w,a[x+2]=c,a[x+3]=m}return{data:a,width:r,height:t,format:"rgba32float"}}function et(e,t,r,n){if(r<=0||n<=0||t.width<=0||t.height<=0)return{x:0,y:0,w:1,h:1};const o=Math.min(t.width/r,t.height/n),a=r*o,v=n*o,h=(t.width-a)/2,f=(t.height-v)/2,w=Math.max(e.zoom,1e-6),c=1/w,m=1/w,x=(h*(1-w)-e.pan.x)/(a*w),i=(f*(1-w)-e.pan.y)/(v*w);return{x,y:i,w:c,h:m}}const dr={zoom:1,pan:{x:0,y:0}};function tt(e){var Q,re;const t=ar(e),r=l.useRef(null),n=l.useRef(null),o=l.useRef(null),a=l.useRef(null),[v,h]=l.useState(!1),[f,w]=l.useState(null),[c,m]=l.useState(0),[x,i]=l.useState(0),p=l.useRef(null),s=l.useRef(null),[u,g]=l.useState(0),[b,d]=l.useState(e.pixelValueNotation??"decimal"),[y,T]=l.useState(!1),A=e.zoom??1,I=e.pan??{x:0,y:0},G=e.onViewportChange,O=t?"none":e.colormap??"none";l.useEffect(()=>{const F=r.current;if(!F)return;let E=!1;return ir(F).then(C=>{if(E){Qe(C);return}a.current=C,h(!0)}),()=>{E=!0,a.current&&(Qe(a.current),a.current=null)}},[]);const{containerProps:B}=Ve({containerRef:n,zoom:A,pan:I,onViewportChange:G}),V=l.useCallback(()=>{G==null||G(dr)},[G]);l.useEffect(()=>{const F=n.current;if(!F)return;const E=new ResizeObserver(()=>i(C=>C+1));return E.observe(F),()=>E.disconnect()},[]),l.useEffect(()=>{const F=n.current;if(!F)return;const E=new IntersectionObserver(C=>{const _=C[0];if(!_)return;const R=a.current;R&&(R.setVisible(_.isIntersecting),_.isIntersecting?R.isParked&&(R.restore(),i(P=>P+1)):R.park())},{threshold:0});return E.observe(F),()=>E.disconnect()},[]),l.useEffect(()=>{var C;if(!t||!v)return;const F=e.hdr;p.current=F;const E=lr(F);(C=a.current)==null||C.setSource(E),w(_=>_&&_.w===E.width&&_.h===E.height?_:{w:E.width,h:E.height}),g(_=>_+1),m(_=>_+1)},[t,v,t?e.hdr:null]),l.useEffect(()=>{if(t||!v)return;const F=e,E=F.imageUrl,C=F.colormap??"none";if(!E){s.current=null,w(null),g(R=>R+1);return}let _=!1;return Oe(E).then(R=>{var k,H;if(_||!R)return;let P=R;if(C!=="none"){const X=`gpu::${E}::${C}`,K=Mt(X);if(K)P=K;else{const ne=Pt.has(C)?"positive":"linear";P=At(R,C,ne),Gt(X,P)}}s.current=R;const D={data:P.data,width:P.width,height:P.height,format:"rgba8unorm"};(k=a.current)==null||k.setSource(D),w(X=>X&&X.w===P.width&&X.h===P.height?X:{w:P.width,h:P.height}),(H=F.onNaturalSize)==null||H.call(F,P.width,P.height),g(X=>X+1),m(X=>X+1)}),()=>{_=!0}},[t,v,t?null:e.imageUrl,t?null:e.colormap]);const L=t?e.exposure??0:0,U=t?e.tonemap:void 0,z=t?e.gamma:void 0;l.useEffect(()=>{const F=a.current;if(!F||!v||!f)return;const E=n.current,C=E?E.getBoundingClientRect():{width:f.w,height:f.h};let _=et({zoom:A,pan:I},C,f.w,f.h);F.backend==="webgl2"&&(_={x:_.x,y:_.y+_.h,w:_.w,h:-_.h});const R=t?{exposureEV:L,operator:cr(U),gamma:z,isScalar:!1,hdrOut:!1,uv:_}:{exposureEV:0,operator:"linear",gamma:1,isScalar:!1,hdrOut:!1,uv:_};F.render(R)},[v,f,c,A,I.x,I.y,L,U,z,x,t]);const N=l.useCallback((F,E,C)=>{if(t){const K=p.current,ne=f;if(!K||!ne||F<0||E<0||F>=ne.w||E>=ne.h)return null;const le=K.shape.length===2?1:K.shape[2]??1,q=(E*ne.w+F)*le,se=K.data,rt=.5;return le===1?{lines:[Z(se[q]??0,"unit",C)],luminance:rt}:{lines:[Z(se[q]??0,"unit",C),Z(se[q+1]??0,"unit",C),Z(se[q+2]??0,"unit",C)],luminance:rt,colors:[ee[0],ee[1],ee[2]]}}const _=s.current;if(!_||F<0||E<0||F>=_.width||E>=_.height)return null;const R=(E*_.width+F)*4,P=_.data[R],D=_.data[R+1],k=_.data[R+2],H=(.299*P+.587*D+.114*k)/255;return O!=="none"||P===D&&D===k?{lines:[Z(P,"uint8",C)],luminance:H}:{lines:[Z(P,"uint8",C),Z(D,"uint8",C),Z(k,"uint8",C)],luminance:H,colors:[ee[0],ee[1],ee[2]]}},[t,f,O]),Y=e.showAxes??!1,j=t?e.label??"":e.label,$=e.interpolation??"auto",te=$==="auto"?void 0:$,W=t?void 0:e.overlay,J=t?void 0:e.overlaySettings,ae=t?!1:e.isDraggable??!1,ue=t?void 0:e.onDragStart;return S.jsxs("div",{className:"relative flex flex-col h-full","data-gpu-image-pane":!0,"data-gpu-backend-ready":v,children:[S.jsxs("div",{ref:n,className:"relative flex-1 min-h-0 min-w-0 flex items-center justify-center overflow-hidden rounded cairn-checkerboard",style:{padding:Y&&f?"16px 4px 4px 28px":"4px",...B.style},onPointerDown:B.onPointerDown,onPointerMove:B.onPointerMove,onPointerUp:B.onPointerUp,onPointerCancel:B.onPointerCancel,onDoubleClick:V,"data-gpu-image-viewport":!0,children:[S.jsxs("div",{ref:o,className:"relative w-full h-full",children:[S.jsx("canvas",{ref:r,className:"w-full h-full object-contain block",style:{imageRendering:te},"data-gpu-image-canvas":!0}),Y&&f&&S.jsx(It,{naturalWidth:f.w,naturalHeight:f.h,zoom:A,containerRef:o}),W&&(J==null?void 0:J.enabled)&&f&&((((Q=W.boxes)==null?void 0:Q.length)??0)>0||(((re=W.masks)==null?void 0:re.length)??0)>0)&&S.jsx(Nt,{data:W,settings:J,naturalWidth:f.w,naturalHeight:f.h})]}),f&&S.jsx(fe,{imageElRef:r,naturalWidth:f.w,naturalHeight:f.h,zoom:A,pan:I,sample:N,notation:b,version:u,onActiveChange:T}),y&&S.jsx(ze,{notation:b,onChange:d})]}),j?S.jsx(Ut,{label:j,isDraggable:ae,onDragStart:ue}):null]})}const fr={zoom:1,pan:{x:0,y:0}};function mr(e){const t=Fe(e),r=new Float32Array(256*4);for(let n=0;n<256;n++)r[n*4+0]=t[n*3+0]/255,r[n*4+1]=t[n*3+1]/255,r[n*4+2]=t[n*3+2]/255,r[n*4+3]=1;return r}function pr({imageUrl:e,baselineUrl:t,mode:r,splitPosition:n,blendAlpha:o,onSplitPositionChange:a,diffSubmode:v,colormap:h="none",zoom:f,pan:w,onViewportChange:c,interpolation:m="auto",label:x="",pixelValueNotation:i="decimal"}){const p=l.useRef(null),s=l.useRef(null),u=l.useRef(null),[g,b]=l.useState(!1),[d,y]=l.useState(null),[T,A]=l.useState(0),[I,G]=l.useState(0),[O,B]=l.useState(null),[V,L]=l.useState(i),[U,z]=l.useState(!1),N=l.useRef(null),Y=l.useRef(null),[j,$]=l.useState(0);l.useEffect(()=>{const E=s.current;if(!E)return;let C=!1;return xe().then(_=>{if(C)return;const R=_.backend==="webgl2",P=R?pe():_,D=P.createSurface(E,{hdr:!1});u.current={device:P,ownsDevice:R,surface:D,texA:null,texB:null},b(!0)}),()=>{var R,P;C=!0;const _=u.current;_&&((R=_.texA)==null||R.destroy(),(P=_.texB)==null||P.destroy(),_.ownsDevice&&_.device.destroy(),u.current=null)}},[]),l.useEffect(()=>{const E=p.current;if(!E)return;const C=new ResizeObserver(()=>G(_=>_+1));return C.observe(E),()=>C.disconnect()},[]),l.useEffect(()=>{if(!g)return;let E=!1;if(!u.current)return;async function _(R){return R?Oe(R):null}return Promise.all([_(e),_(t)]).then(([R,P])=>{var K,ne,le;if(E||!u.current)return;const D=u.current;N.current=R,Y.current=P,(K=D.texA)==null||K.destroy(),(ne=D.texB)==null||ne.destroy(),D.texA=null,D.texB=null;const k=R??P;if(!k){y(null),$(q=>q+1);return}const H=q=>{const se=D.device.createTexture(q.width,q.height,"rgba8unorm");return se.write(q.data),se};D.texA=H(R??k),D.texB=H(P??k);const X=s.current;X.width=k.width,X.height=k.height,(le=D.surface)==null||le.configure(k.width,k.height),y({w:k.width,h:k.height}),$(q=>q+1),A(q=>q+1)}),()=>{E=!0}},[g,e,t]);const te=l.useMemo(()=>(v??"").includes("signed")?"signed":"positive",[v]),W=l.useMemo(()=>h!=="none"?mr(h):void 0,[h]);l.useEffect(()=>{const E=u.current;if(!g||!E||!E.surface||!E.texA||!E.texB||!d)return;const C=p.current,_=C?C.getBoundingClientRect():{width:d.w,height:d.h};let R=et({zoom:f,pan:w},_,d.w,d.h);E.device.backend==="webgl2"&&(R={x:R.x,y:R.y+R.h,w:R.w,h:-R.h});const P={exposureEV:0,operator:"linear",gamma:1,uv:R,mode:r,split:n,alpha:o,diffSubmode:v??"absolute",diffCmapMode:te,diffColormap:r==="diff"?W:void 0};Qt(E.device,E.surface,E.texA,E.texB,P)},[g,d,T,f,w.x,w.y,r,n,o,v,te,W,I]),l.useEffect(()=>{const E=u.current;if(!g||!E||!E.texA||!E.texB||!t){B(null);return}let C=!1;return er(E.device,E.texA,E.texB).then(_=>{C||B(_)}),()=>{C=!0}},[g,T,t]);const J=E=>(C,_,R)=>{const P=E.current;if(!P||C<0||_<0||C>=P.width||_>=P.height)return null;const D=(_*P.width+C)*4,k=P.data[D],H=P.data[D+1],X=P.data[D+2],K=(.299*k+.587*H+.114*X)/255;return k===H&&H===X?{lines:[Z(k,"uint8",R)],luminance:K}:{lines:[Z(k,"uint8",R),Z(H,"uint8",R),Z(X,"uint8",R)],luminance:K,colors:[ee[0],ee[1],ee[2]]}},ae=l.useMemo(()=>J(N),[]),ue=l.useMemo(()=>J(Y),[]),{containerProps:Q}=Ve({containerRef:p,zoom:f,pan:w,onViewportChange:c}),re=l.useCallback(()=>c==null?void 0:c(fr),[c]),F=m==="auto"?void 0:m;return S.jsxs("div",{className:"relative flex flex-col h-full","data-gpu-compare-pane":!0,"data-gpu-compare-ready":g,children:[S.jsxs("div",{ref:p,className:"relative flex-1 min-h-0 min-w-0 flex items-center justify-center overflow-hidden rounded cairn-checkerboard",style:{padding:0,...Q.style},onPointerDown:Q.onPointerDown,onPointerMove:Q.onPointerMove,onPointerUp:Q.onPointerUp,onPointerCancel:Q.onPointerCancel,onDoubleClick:re,"data-gpu-compare-viewport":!0,children:[S.jsxs("div",{className:"relative w-full h-full",children:[S.jsx("canvas",{ref:s,className:"w-full h-full object-contain block",style:{imageRendering:F},"data-gpu-compare-canvas":!0}),r==="split"&&S.jsx("div",{className:"absolute top-0 bottom-0 z-20 flex items-center",style:{left:`${n*100}%`,transform:"translateX(-50%)",cursor:"col-resize"},onDoubleClick:E=>{E.stopPropagation(),a==null||a(.5)},onPointerDown:E=>{E.stopPropagation(),E.preventDefault();const _=E.currentTarget.parentElement.getBoundingClientRect(),R=D=>{a==null||a(Math.max(0,Math.min(1,(D.clientX-_.left)/_.width)))},P=()=>{window.removeEventListener("pointermove",R),window.removeEventListener("pointerup",P)};window.addEventListener("pointermove",R),window.addEventListener("pointerup",P)},children:S.jsx("div",{className:"w-1 h-full bg-accent/80 rounded-full"})})]}),r==="split"?S.jsxs(S.Fragment,{children:[t&&d&&S.jsx("div",{className:"absolute inset-0 overflow-hidden pointer-events-none",style:{clipPath:`inset(0 ${(1-n)*100}% 0 0)`},children:S.jsx(fe,{imageElRef:s,naturalWidth:d.w,naturalHeight:d.h,zoom:f,pan:w,sample:ae,notation:V,version:j})}),t&&d&&S.jsx("div",{className:"absolute inset-0 overflow-hidden pointer-events-none",style:{clipPath:`inset(0 0 0 ${n*100}%)`},children:S.jsx(fe,{imageElRef:s,naturalWidth:d.w,naturalHeight:d.h,zoom:f,pan:w,sample:ue,notation:V,version:j,onActiveChange:z})})]}):d&&S.jsx(fe,{imageElRef:s,naturalWidth:d.w,naturalHeight:d.h,zoom:f,pan:w,sample:ae,notation:V,version:j,onActiveChange:z}),U&&S.jsx(ze,{notation:V,onChange:L})]}),S.jsx("span",{className:"absolute top-1 left-1 z-10 rounded bg-accent/20 px-1 py-0.5 text-[10px] text-accent backdrop-blur-sm",children:"REF"}),x?S.jsx("span",{className:"absolute bottom-1 right-1 z-10 rounded bg-bg/80 px-1 py-0.5 text-[10px] text-fg-muted backdrop-blur-sm",children:x}):null,O&&S.jsxs("span",{className:"absolute top-1 right-1 z-10 rounded bg-bg/80 px-1 py-0.5 text-[10px] text-fg-muted backdrop-blur-sm font-mono","data-gpu-compare-metrics":!0,children:["MSE ",O.mse.toExponential(2)," · PSNR ",Number.isFinite(O.psnr)?O.psnr.toFixed(1):"∞"," dB · MAE"," ",O.mae.toExponential(2)]})]})}async function gr(){if(!window.__cairnPlotGpuImageLoaded){if(window.__cairnPlotUseGpuImage===!1){console.info("cairn-plot gpu-image addon: skipped (__cairnPlotUseGpuImage === false)");return}if(typeof window.__cairnPlotRegisterRenderer!="function"){console.error("cairn-plot gpu-image addon: core bundle not installed (window.__cairnPlotRegisterRenderer missing) — staying on legacy panes.");return}try{await xe(),window.__cairnPlotRegisterRenderer("image",tt),window.__cairnPlotRegisterRenderer("imagehdr",tt),window.__cairnPlotGpuComparePane=pr,window.__cairnPlotGpuImageLoaded=!0}catch(e){console.warn("cairn-plot gpu-image addon: engine init failed, staying on legacy panes",e)}}}gr()})(__cairnPlotJsxRuntime,__cairnPlotReact);
