...
initPyodide();
<\/script>
</body></html>`}function te({runId:r,metric:s,settingsKeyOverride:y,onRemove:v}){const[d,f]=J(y??{runId:r,metricName:s.name,contextHash:s.context_hash},K),E=z(r,s.name,{context:s.context_hash||void 0,maxPoints:200}),b=a.useMemo(()=>{var e;return(((e=E.data)==null?void 0:e.points)??[]).filter(t=>t.artifact_hash)},[E.data]),h=a.useMemo(()=>{const e=new Set;for(const t of b)e.add(t.step);return Array.from(e).sort((t,n)=>t-n)},[b]),[H,I]=a.useState(d.sliderStep??0),_=Math.min(Math.max(0,H),Math.max(0,h.length-1)),m=h[_]??0,i=a.useMemo(()=>b.find(e=>e.step===m)??b[0],[b,m]),l=a.useMemo(()=>A((i==null?void 0:i.artifact_metadata)??null)??{},[i]),S=a.useRef(null),[O,F]=a.useState(300),[M,w]=a.useState(null),[R,N]=a.useState(!1),C=a.useRef("");a.useEffect(()=>()=>{x.current&&URL.revokeObjectURL(x.current)},[]),a.useEffect(()=>{const e=t=>{var n;((n=t.data)==null?void 0:n.type)==="cairn:resize"&&typeof t.data.height=="number"&&F(Math.max(100,Math.min(2e3,t.data.height)))};return window.addEventListener("message",e),()=>window.removeEventListener("message",e)},[]);const c=l.plugin_lang??"js",j=a.useRef(null),[k,D]=a.useState(null);a.useEffect(()=>{if(c!=="server"&&c!=="window"||!(i!=null&&i.artifact_hash)||!l.plugin_hash)return;const t=`${window.location.protocol==="https:"?"wss:":"ws:"}//${window.location.host}/ws/plugin/${r}/${encodeURIComponent(s.name)}`,n=new WebSocket(t);j.current=n,n.onopen=()=>{n.send(JSON.stringify({type:"render",artifact_hash:i.artifact_hash,metadata:l,step:m}))};let u="";return n.onmessage=p=>{if(typeof p.data=="string"){const g=JSON.parse(p.data);g.type==="frame"?u=g.mime||"image/png":g.type==="error"&&w(g.message)}else if(p.data instanceof Blob){const g=new Blob([p.data],{type:u}),W=URL.createObjectURL(g);D(U=>(U&&URL.revokeObjectURL(U),W))}},n.onerror=()=>w("WebSocket connection failed"),()=>{n.close(),j.current=null}},[c,i,l,m,r,s.name]);const P=a.useCallback((e,t)=>{const n=j.current;if(!n||n.readyState!==WebSocket.OPEN)return;const u=t.target.getBoundingClientRect();n.send(JSON.stringify({type:"mouse",x:Math.round(t.clientX-u.left),y:Math.round(t.clientY-u.top),button:t.button,action:e}))},[]),x=a.useRef(""),L=a.useCallback(async()=>{if(!(c==="server"||c==="window")&&l.plugin_hash){w(null);try{const e=await X(l.plugin_hash),t=S.current;if(!t)return;const n=`${l.plugin_hash}:${c}`;if(C.current!==n){if(C.current=n,N(!1),c==="js")t.removeAttribute("src"),t.srcdoc=V(e);else{t.removeAttribute("srcdoc"),x.current&&URL.revokeObjectURL(x.current);const u=Z(e),p=new Blob([u],{type:"text/html"}),g=URL.createObjectURL(p);x.current=g,t.src=g}await new Promise(u=>{t.onload=()=>u()}),N(!0)}}catch(e){w(e instanceof Error?e.message:String(e))}}},[l.plugin_hash,c]);a.useEffect(()=>{L()},[L]),a.useEffect(()=>{if(!R||!(i!=null&&i.artifact_hash))return;const e=S.current;if(!(e!=null&&e.contentWindow))return;let t=!1;return Q(i.artifact_hash).then(n=>{var p;if(t)return;const u=n.slice(0);(p=e.contentWindow)==null||p.postMessage({type:"cairn:render",data:u,metadata:l,step:m,runId:r,metricName:s.name},"*",[u])}).catch(n=>{t||w(n instanceof Error?n.message:String(n))}),()=>{t=!0}},[R,i,l,m,r,s.name]);const q=h.length>0?`step ${m} (${_+1}/${h.length})`:l.plugin_name??"plugin";return o.jsxs("div",{className:"card p-4 flex flex-col",style:{position:"relative",height:d.collapsed?void 0:d.height??400,gridColumn:(d.colSpan??1)>1?`span ${d.colSpan}`:void 0},children:[o.jsx(G,{title:d.title??s.name,onTitleChange:e=>f({title:e||void 0}),subtitle:q,collapsed:d.collapsed,onToggleCollapse:()=>f({collapsed:!d.collapsed}),onToggleFullWidth:()=>f({colSpan:(d.colSpan??1)>1?1:2}),isFullWidth:(d.colSpan??1)>1,onRemove:v,children:o.jsx("span",{className:"inline-flex items-center rounded bg-bg-hover px-1.5 py-0.5 text-[10px] text-fg-muted",children:c==="window"?"Window":c==="server"?"Server":c==="py"?"Python":"JS"})}),!d.collapsed&&o.jsxs(o.Fragment,{children:[M?o.jsx("div",{className:"flex-1 rounded bg-bg p-3 text-xs text-status-failed overflow-auto",children:o.jsx("pre",{children:M})}):l.plugin_hash?c==="server"||c==="window"?k?o.jsx("img",{src:k,alt:`Server plugin: ${l.plugin_name??s.name}`,className:"flex-1 w-full rounded object-contain cursor-grab active:cursor-grabbing",style:{minHeight:200,userSelect:"none"},draggable:!1,onMouseDown:e=>{e.preventDefault(),P("down",e)},onMouseMove:e=>P("move",e),onMouseUp:e=>P("up",e),onMouseLeave:e=>P("up",e)}):o.jsxs("div",{className:"flex-1 flex items-center justify-center",children:[o.jsx("div",{className:"h-8 w-8 motion-safe:animate-spin rounded-full border-2 border-accent border-t-transparent"}),o.jsx("span",{className:"ml-2 text-xs text-fg-muted",children:"Connecting to server..."})]}):o.jsx("iframe",{ref:S,...c==="js"?{sandbox:"allow-scripts"}:{},className:"flex-1 w-full rounded border-0",style:{height:O,minHeight:200},title:`Plugin: ${l.plugin_name??s.name}`}):o.jsx("div",{className:"flex-1 flex items-center justify-center text-sm text-fg-muted",children:"No plugin metadata found"}),h.length>1&&o.jsx("input",{type:"range",min:0,max:h.length-1,value:_,onChange:e=>{const t=Number(e.target.value);I(t),f({sliderStep:t})},className:"mt-3 w-full accent-accent"})]}),o.jsx(Y,{height:d.height,onHeightChange:e=>f({height:e}),colSpan:d.colSpan??1,onColSpanChange:e=>f({colSpan:e})})]})}export{te as default};
